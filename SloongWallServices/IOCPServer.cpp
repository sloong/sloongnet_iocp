...
void CIOCPServer::FreeBuffers()
{
	...
	while (pFreeBuffer != NULL)
	{
		...
		{
#ifdef _DEBUG
			::OutputDebugString(_T("  FreeBuffers释放内存出错！"));
#endif // _DEBUG
			break;
		}
		else
		{
#ifdef _DEBUG
			OutputDebugString(_T("  FreeBuffers释放内存！"));
#endif // _DEBUG
		}
...
{
#ifdef _DEBUG
	::OutputDebugString(_T("	WorkerThread 启动... \n"));
#endif // _DEBUG
	...
	while (TRUE)
	{
		...
		{
#ifdef _DEBUG
			::OutputDebugString(_T("	WorkerThread 退出 \n"));
#endif // _DEBUG
			::ExitThread(0);
		}
...
void CIOCPServer::HandleIO(DWORD dwKey, CIOCPBuffer *pBuffer, DWORD dwTrans, int nError)
{
	...
	if (pContext != NULL)
	{
		...
		{
#ifdef _DEBUG
			::OutputDebugString(_T("	检查到套节字已经被我们关闭 \n"));
#endif // _DEBUG
...
#ifdef _DEBUG
		::OutputDebugString(_T("	检查到套节字连接超时未完成被我们关闭 \n"));
#endif // _DEBUG
...
#ifdef _DEBUG
			::OutputDebugString(_T("	检查到客户套节字上发生错误 \n"));
#endif // _DEBUG
		}
...
#ifdef _DEBUG
			::OutputDebugString(_T("	检查到监听套节字上发生错误 \n"));
#endif // _DEBUG
		}
	...
	if (pBuffer->nOperation == OP_ACCEPT)
	{
		...
		{
#ifdef _DEBUG
			::OutputDebugString(_T("	监听套节字上客户端关闭 \n"));
#endif // _DEBUG
		...
		else
		{
			...
			if (pClient != NULL)
			{
				if (AddAConnection(pClient))
				{
					...
					// 关联新连接到完成端口对象
					::CreateIoCompletionPort((HANDLE)pClient->s, m_hCompletion, (ULONG_PTR)pClient, 0);
