...
//Open
void CRecordset::Open(wstring Source,CConnection & ActiveConn,CursorTypeEnum CursorType,LockTypeEnum LockType,long Options)
{
    m_pRst->Open(_variant_t(Source.c_str()),ActiveConn.GetConnection().GetInterfacePtr(),CursorType,LockType,Options);
}


void CRecordset::Open(wstring Source,_ConnectionPtr pConn,CursorTypeEnum CursorType, LockTypeEnum LockType,long Options)
{
	m_pRst->Open(_variant_t(Source.c_str()),pConn.GetInterfacePtr(),CursorType,LockType,Options);	
}
...
//GetString
wstring CRecordset::GetString(wstring columnName)
{
	try
	{
		return (LPWSTR)(_bstr_t)m_pRst->GetCollect(_variant_t(columnName.c_str()));
	}
	catch(_com_error e)
	{
		throw e;	
...
//GetInt
int CRecordset::GetInt(wstring columnName)
{
	try
	{
		return (short)m_pRst->GetCollect(_variant_t(columnName.c_str()));
	}
	catch(_com_error e)
	{
		throw e;	
